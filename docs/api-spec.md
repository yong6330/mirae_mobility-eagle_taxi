...
| 장소명 검색 및 좌표 선택 | 포함 | Kakao Maps JavaScript API, Places keywordSearch |
...
| 지도 표시 | API 일부 포함 | Kakao Map + 파티 상세 좌표값 |
...
| 422 | 검증 실패 | 데이터 타입 오류 |
| 500 | 서버 오류 | 예외 처리 필요 |
| 502 | 외부 API 오류 | Kakao Mobility API 오류, 요금 산정 실패 |
...
예상 택시비는 출발지 좌표, 도착지 좌표, 출발 시간을 기준으로 백엔드가 Kakao Mobility 길찾기 API를 호출해 자동 산정한다.

Kakao Mobility API 응답에서 아래 값을 사용한다.

<!-- 표 10 -->
| Kakao Mobility 응답값 | 프로젝트 저장/응답 필드 | 설명 |
| --- | --- | --- |
...
백엔드는 출발지 좌표, 도착지 좌표, 출발 시간을 기준으로 Kakao Mobility API를 호출한다.
...
fare_source는 kakao_mobility로 저장한다.
...
Kakao Mobility REST API Key는 백엔드에서만 사용한다.

프론트엔드에 Kakao Mobility REST API Key를 노출하지 않는다.
...
| 항목 | 기준 |
| --- | --- |
| estimated_fare | Kakao Mobility API로 자동 산정된 전체 예상 택시비 |
...
| 도착지 좌표 없음 | 파티 생성 차단 |
| 출발 시간 없음 | 파티 생성 차단 |
| Kakao Mobility API Key 없음 | 파티 생성 차단 |
| 경로 조회 실패 | 파티 생성 차단 |
...
파티 참여 또는 참여 취소 시에는 Kakao Mobility API를 다시 호출하지 않는다.
...
estimated_fare는 사용자가 입력한 값이 아니라 Kakao Mobility API로 자동 산정한 값이다.
...
| PLACE-001 | 장소명 검색 및 좌표 선택 | Kakao SDK | kakao.maps.services.Places.keywordSearch | Kakao JS Key | Must |
...
| --- | --- |
| 기능 | 장소명 검색 및 좌표 선택 |
| Method | Kakao Maps JavaScript API |
| Endpoint | 백엔드 Endpoint 없음 |
| 사용 SDK | kakao.maps.services.Places.keywordSearch |
| 인증 | Kakao JavaScript API Key |
| 사용 화면 | 파티 생성 /parties/new, 파티 목록·검색 /parties, 파티 상세 지도 표시 |
| 처리 내용 | 사용자가 입력한 장소명을 Kakao Maps Places 검색으로 조회하고, 선택한 장소의 장소명·주소·위도·경도를 프론트에서 확보한다. |
...
React 프론트엔드에서 Kakao Maps JavaScript API를 호출하는 외부 SDK 기능이다.
...
프론트엔드는 Kakao Maps Places keywordSearch를 호출한다.
...
| Kakao Maps API 로딩 실패 | “지도를 불러오지 못했습니다.” 표시 |
| 좌표값 없음 | 파티 생성 불가 |
| 장소 미선택 | 필수값 누락 안내 |
...
장소 검색과 좌표 선택은 프론트엔드에서 Kakao Maps JavaScript API로 처리한다.

백엔드는 사용자가 선택한 장소명과 좌표값을 저장한다.

사용자에게 경로 안내·내비게이션·실시간 이동 경로를 제공하는 기능은 구현하지 않는다.

예상 택시비 산정을 위한 백엔드 내부 경로 조회는 FARE-001 및 PARTY-001 내부 로직에서 Kakao Mobility API로 처리한다.
...
| 처리 내용 | 출발지·도착지 좌표와 출발 시간을 기준으로 Kakao Mobility 길찾기 API를 호출해 예상 택시비, 통행료, 이동 거리, 소요 시간을 산정한다. |
...
백엔드는 Kakao Mobility 길찾기 API를 직접 호출한다.

프론트엔드에서 Kakao Mobility REST API Key를 직접 사용하지 않는다.
...
| 항목 | 내용 |
| --- | --- |
| 외부 API | Kakao Mobility Directions API |
| 호출 주체 | FastAPI 백엔드 |
| 인증 방식 | Authorization: KakaoAK ${KAKAO_MOBILITY_REST_API_KEY} |
...
Kakao Mobility API 요청값 생성

Kakao Mobility 길찾기 API 호출
...
| 502 | Kakao Mobility API 오류 | 예상 택시비를 계산하지 못했습니다. |
...
Kakao Mobility REST API Key는 백엔드 .env에 저장한다.
...
| 처리 내용 | 사용자가 선택한 출발지, 도착지, 출발 시간, 만남 장소, 최대 인원, 성별 매칭 옵션을 기준으로 새 파티를 생성한다. 예상 택시비는 백엔드가 Kakao Mobility 길찾기 API로 자동 산정한다. |
...
Kakao Mobility 길찾기 API로 예상 택시비, 통행료, 거리, 소요시간 산정
...
fare_source를 kakao_mobility로 저장
...
파티 목록 조회 시 Kakao Mobility API를 다시 호출하지 않는다.
...
파티 참여 시 Kakao Mobility API를 다시 호출하지 않는다.
...
참여 취소 시 Kakao Mobility API를 다시 호출하지 않는다.
...
| VITE_KAKAO_MAP_API_KEY | 프론트엔드 | Kakao Maps JavaScript API Key |
...
| KAKAO_MOBILITY_REST_API_KEY | 백엔드 | Kakao Mobility 길찾기 API 호출용 REST API Key |

주의사항:

VITE_KAKAO_MAP_API_KEY는 프론트에서 사용하는 JavaScript Key이다.
...
□ GET /api/places가 백엔드 API에서 제거되고 Kakao Places 기준으로 정리되었는가

□ 장소명 검색은 Kakao Maps JavaScript API 기준으로 작성되었는가
...
□ Kakao Mobility REST API Key가 백엔드 환경변수로 분리되었는가

□ 프론트에서 Kakao Mobility REST API Key를 직접 사용하지 않는다고 명시되었는가

□ estimated_fare는 Kakao Mobility API의 summary.fare.taxi 기준인가

□ toll_fare, distance_meters, duration_seconds가 함께 저장되는가

□ fare_source가 kakao_mobility 기준으로 정리되었는가
