...
- 외부 요금 Key가 없거나 호출 실패면 모든 값 0 + `fare_source: "fallback"`. 정상이면 `fare_source: "external_mobility"`.
...
fare_source: "external_mobility" | "fallback"
...
| 요금 출처 fare_source | external_mobility / fallback |
...
- `fare_source` 값은 `"external_mobility"` (이전 `"external"` 폐기).

아래는 임의 확정하지 않고 팀 회의 결정 후 갱신한다.
- 환경변수 이름: 코드 `.env`는 `MOBILITY_REST_API_KEY`, `JWT_EXPIRES_MINUTES`. 명세 §23-2/README는 `MOBILITY_REST_API_KEY`, `ACCESS_TOKEN_EXPIRE_MINUTES`. 회의 후 통일.
...
- 외부 요금 Key 미설정 → **500 `FARE_CONFIG_MISSING`**
- 외부 요금 호출/경로 실패 → **502 `FARE_UPSTREAM_FAILED` / `FARE_ROUTE_NOT_FOUND`**
...
### 10-5. 신규 관리자 엔드포인트
- `GET /api/admin/system/status` (ADMIN-011) — 서버/DB/외부 요금 API/fallback/서버시간.
