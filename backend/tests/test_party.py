...
def test_create_party_success(client):
    ...
    # fare_source는 실키 유무에 따라 external_mobility/fallback 양쪽 모두 허용한다 (요금 로직은 test_fares.py에서 별도 검증).
    assert body["fare_source"] in ("external_mobility", "fallback")
...
def test_join_party_success_updates_members_and_per_person_fare(client):
    ...
    # members는 평탄 구조에 참여 시각을 포함한다.
    assert set(body["members"][0].keys()) == {"id", "name", "gender", "joined_at"}
