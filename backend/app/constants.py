...
class FareSource:
    ...
    EXTERNAL_MOBILITY = "external_mobility"  # 외부 길찾기 요금 API 응답 기반
    FALLBACK = "fallback"  # API Key 없거나 호출 실패 시 0원 처리
    ADMIN_OVERRIDE = "admin_override"  # 관리자 수동 보정 (ADMIN-025)

    ALL: tuple[str, ...] = (EXTERNAL_MOBILITY, FALLBACK, ADMIN_OVERRIDE)


FareSourceType = Literal["external_mobility", "fallback", "admin_override"]
