...
from app.errors import STATUS_DEFAULT_CODE, ErrorCode
...
async def http_exception_handler(
    ...
    """모든 HTTPException을 명세 §9 형식 {detail, error_code}로 직렬화한다.

    AppError는 .error_code를 직접 싣고, 일반 HTTPException은 상태코드 기본값으로 보정한다.
    """
    error_code = getattr(exc, "error_code", None) or STATUS_DEFAULT_CODE.get(
